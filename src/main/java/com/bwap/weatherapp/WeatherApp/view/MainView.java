...
public class MainView<cityName> extends UI {
    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        searchButton.addClickListener(clickEvent -> {
            if(!checkIfInt(cityTextField.getValue())){
                Notification.show("Error - Please enter the city");
            }
            else{
    ...
    private boolean checkIfInt(String cityName){
        try {
            int value = Integer.parseInt(cityName);
            return false;
    ...
    private void setHeader(){
        ...
        Label Title = new Label("Weather forecasting application");
    ...
    private void setForm(){
        ...
        stateTextField.setWidth("80%");
        stateTextField.setPlaceholder("enter state name");
        formLayout.addComponents(stateTextField);

        countryTextField = new TextField();
        countryTextField.setWidth("80%");
        countryTextField.setPlaceholder("enter country name");
        formLayout.addComponents(countryTextField);
    ...
    private void dashboardTitle() {
        ...
        //City Location
        location = new Label("Currently in Noida");
    ...
    private void footer(){
        ...
        description.setValue("Weather App by Aditi and Team.");
    ...
    private void updateUI() throws JSONException {
        weatherService.geoTag();

        //Getting City name from text field and assignig it to API URL
        String city = cityTextField.getValue();
        String state = stateTextField.getValue();
        String country = countryTextField.getValue();

        String defaultUnit;
        weatherService.setCityName(city);
        weatherService.setState(state);
        weatherService.setCountry(country);
        ...
        location.setValue("Currently in "+placeName);
